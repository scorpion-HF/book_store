...
class AddToCartView(LoginRequiredMixin, RedirectView, SingleObjectMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if book in self.request.user.owned_ebooks.all():
            messages.error(request, 'کتاب الکترونیکی "{}" قبلا خریداری شده است'.format(book.title))
        ...
        if not cart_item_created:
            if hasattr(cart_item.item, 'electronicbook'):
                messages.error(request, 'کتاب الکترونیکی "{}" قبلا به سبد خرید اضافه شده است'.format(book.title))
                return self.get(request, *args, **kwargs)
...
def send_request(request, order_id):
    ...
    result = client.service.PaymentRequest(settings.MERCHANT, order.get_total() / 10,
                                           ...
                                           'http://oghyanoosedanesh.ir/orders/verify/{}/'.format(order_id))
    ...
    if request.GET.get('Status') == 'OK':
        ...
        if result.Status == 100:
            ...
            for order_item in order.orderitem_set.all():
                book = Book.objects.get(title=order_item.item_title)
                if hasattr(book, 'electronicbook'):
                    request.user.owned_ebooks.add(book)
